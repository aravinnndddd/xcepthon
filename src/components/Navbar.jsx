...
const Navbar = () => {
    ...
    return (
        <nav
            className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-goku-dark/90 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.5)] border-b border-white/5' : 'bg-transparent'
                }`}
        >
                        ...
                        <div className="flex space-x-8">
                            {navLinks.map((link) => (
                                <a
                                    key={link.name}
                                    href={link.href}
                                    className="text-gray-300 hover:text-white font-sans text-sm tracking-wide uppercase transition-colors relative group"
                                >
                                    {link.name}
